...
DbgUartMode UartMode_BT_Request =	{MODE_BT_REQUEST,	"BT_RQST."};
DbgUartMode UartMode_DT_Request =	{MODE_DT_REQUEST,	"DT_RQST."};
DbgUartMode UartMode_AC_Report =	{MODE_AC_REPORT,	"AC_RPRT."};
DbgUartMode UartMode_Idle =				{MODE_IDLE,				"IDLE...."};
DbgUartMode UartMode_Error =			{MODE_ERROR,			"ERROR..."};


DbgUartFuncState UartFState_PrepareSending =		{FST_PREPARE_SENDING,							"_PRP_SNDG......"};
DbgUartFuncState UartFState_Sending =						{FST_SENDING,											"_SNDNG........."};
DbgUartFuncState UartFState_TermCntSent =				{FST_TRMNL_CNT_SENT,							"_TRMNL_CNT_SENT"};
DbgUartFuncState UartFState_TransmitComplete =	{FST_TRANSMIT_COMPLETE,						"_TRNSMT_CMPLT..."};
DbgUartFuncState UartFState_PrepareReceiving =	{FST_PREPARE_RECEIVING,						"_PRP_RCVNG......"};
DbgUartFuncState UartFState_Receiving =					{FST_RECEIVING,										"_RCVNG.........."};
DbgUartFuncState UartFState_TermCntReceiving =	{FST_TRMNL_CNT_RECEIVED,					"_TRMNL_CNT_RCVD."};
DbgUartFuncState UartFState_Free =							{FST_FREE,												"_FREE..........."};
DbgUartFuncState UartFState_ErrFg =							{FST_ERR_FG,											"_ERR_FG........."};
DbgUartFuncState UartFState_ErrData =						{FST_ERR_DATA,										"_ERR_DATA......."};
DbgUartFuncState UartFState_ErrTimeout =				{FST_ERR_TIMEOUT,									"_ERR_TIMEOUT...."};							
...
//	dbg_ary_check_points.ary_dbg_chk_pts[row_ix][col_ix++] = ' '; //[]
	...
	{
		dbg_ary_check_points.ary_dbg_chk_pts[row_ix][col_ix++] = 'B'; //[2]
		dbg_ary_check_points.ary_dbg_chk_pts[row_ix][col_ix++] = 'T'; //[3]
	}
	else if(phUart->uart_enm == UART_PC)
	{
		dbg_ary_check_points.ary_dbg_chk_pts[row_ix][col_ix++] = 'P'; //[2]
		dbg_ary_check_points.ary_dbg_chk_pts[row_ix][col_ix++] = 'C'; //[3]
	}
	else 
	{
...
//	dbg_ary_check_points.ary_dbg_chk_pts[row_ix][col_ix++] = ' '; //[6]
	...
	{
		// set enum name
		str_size = UART_MODE_NAME_SIZE;// 8 ;
		for(chr_ix = 0; chr_ix < str_size; chr_ix++)
		{
	...
	dbg_ary_check_points.ary_dbg_chk_pts[row_ix][col_ix++]=':'; //[15] or [...]
	
	if(ix_uart_fst < FUNC_SATE_ENUMS_COUNT)
	{
			// set enum name
		str_size = UART_SUBSTATE_NAME_SIZE;// 16 ;
		for(chr_ix = 0; chr_ix < str_size; chr_ix++)
		{
